...
function TaskList({type,day,dateStr,pf,af}:{type:TaskType;day:number;dateStr:string;pf:Priority|'all';af:string}){
  ...
  return(<div><div className="mb-5 flex items-center gap-4"><div className="flex-1 h-3 rounded-full bg-gray-200/80 dark:bg-gray-800/80 overflow-hidden ring-1 ring-gray-200/50 dark:ring-gray-700/50"><div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-blue-600 transition-all duration-700 ease-out shadow-sm shadow-blue-500/20" style={{width:(cc/act.length)*100+'%'}}/></div><span className="text-sm font-semibold text-gray-500 dark:text-gray-400 whitespace-nowrap tabular-nums">{cc}<span className="text-gray-400 dark:text-gray-600">/{act.length}</span> <span className="text-gray-400 dark:text-gray-500 font-medium">·</span> <span className={cc===act.length?'text-emerald-500 dark:text-emerald-400':'text-gray-500 dark:text-gray-400'}>{Math.round((cc/act.length)*100)}%</span></span></div>
    <div className="space-y-3">{act.map(task=>{const ic=cids.has(task._id);const com=comp.find(x=>x.taskId===task._id);const sCls=ic?'opacity-60':'';const bCls=ic?"border-gray-100/30 dark:border-gray-800/30 bg-gray-50/20 dark:bg-gray-900/20":task.priority==='red'?"bg-gradient-to-br from-red-50/60 to-white/80 dark:from-red-950/15 dark:to-gray-900/80 border-red-200/50 dark:border-red-900/40 shadow-sm hover:shadow-md hover:shadow-red-500/5 hover:border-red-300/60 dark:hover:border-red-700/40":task.priority==='amber'?"bg-gradient-to-br from-amber-50/60 to-white/80 dark:from-amber-950/15 dark:to-gray-900/80 border-amber-200/50 dark:border-amber-900/40 shadow-sm hover:shadow-md hover:shadow-amber-500/5 hover:border-amber-300/60 dark:hover:border-amber-700/40":"bg-gradient-to-br from-emerald-50/60 to-white/80 dark:from-emerald-950/15 dark:to-gray-900/80 border-emerald-200/50 dark:border-emerald-900/40 shadow-sm hover:shadow-md hover:shadow-emerald-500/5 hover:border-emerald-300/60 dark:hover:border-emerald-700/40";const pBg=task.priority==='red'?"bg-red-100/80 dark:bg-red-900/25 text-red-600 dark:text-red-400 shadow-sm shadow-red-500/10":task.priority==='amber'?"bg-amber-100/80 dark:bg-amber-900/25 text-amber-600 dark:text-amber-400 shadow-sm shadow-amber-500/10":"bg-emerald-100/80 dark:bg-emerald-900/25 text-emerald-600 dark:text-emerald-400 shadow-sm shadow-emerald-500/10";const pDot=task.priority==='red'?"bg-red-500":task.priority==='amber'?"bg-amber-500":"bg-emerald-500";return(<div key={task._id} className={"group relative overflow-hidden rounded-xl border transition-all duration-300 " + sCls + " " + bCls}>
      <div className={"flex items-center gap-3 sm:gap-4 px-4 sm:px-6 py-3.5 sm:py-4 " + (ic?"opacity-70":"")}>
        <button onClick={()=>ht(task._id,cb[task._id]||task.assignedTo)} className={"flex-shrink-0 h-6 w-6 rounded-md border-2 flex items-center justify-center transition-all duration-200 " + (ic?"bg-gradient-to-br from-blue-500 to-blue-600 border-blue-500 text-white shadow-sm shadow-blue-500/20":"border-gray-300 dark:border-gray-600 hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-sm")}>{ic&&<svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7"/></svg>}</button>
        <span className={"flex-1 min-w-0 text-[15px] sm:text-base font-semibold truncate " + (ic?"text-gray-400 dark:text-gray-500 line-through":"text-gray-900 dark:text-white")}>{task.title}</span>
        <span className={"flex-shrink-0 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[11px] sm:text-xs font-bold uppercase tracking-wider " + pBg}><span className={"w-1.5 h-1.5 rounded-full " + pDot}/>{task.priority==="red"?"High":task.priority==="amber"?"Medium":"Low"}</span>
        {task.link&&<a href={task.link} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} className="flex-shrink-0 inline-flex items-center gap-1.5 rounded-full bg-blue-50/80 dark:bg-blue-900/20 px-3 py-1 text-[11px] sm:text-xs font-medium text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-all hover:shadow-sm"><svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg><span className="hidden sm:inline">Link</span></a>}
        <select value={cb[task._id]||task.assignedTo} onChange={e=>setCb(prev=>({...prev,[task._id]:e.target.value}))} onClick={e=>e.stopPropagation()} className={"flex-shrink-0 text-[11px] sm:text-xs rounded-lg border px-2.5 sm:px-3 py-1.5 font-medium transition-all min-w-[90px] sm:min-w-[100px] " + (ic?"border-gray-200/50 dark:border-gray-700/50 text-gray-400 dark:text-gray-500 bg-gray-50/50 dark:bg-gray-800/50":"border-gray-200/70 dark:border-gray-700/70 text-gray-500 dark:text-gray-400 bg-white/50 dark:bg-gray-800/50 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50/50 dark:hover:bg-blue-900/10")}>{ae.map(n=><option key={n} value={n}>{n}</option>)}</select>
        {ic&&com&&<span className="flex-shrink-0 inline-flex items-center gap-1 rounded-full bg-emerald-100/60 dark:bg-emerald-900/20 px-2.5 py-1 text-[11px] sm:text-xs font-semibold text-emerald-600 dark:text-emerald-400 whitespace-nowrap"><svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7"/></svg>{com.completedBy}</span>}
