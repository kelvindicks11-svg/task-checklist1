...
function TaskList({type,day,dateStr,pf,af}:{type:TaskType;day:number;dateStr:string;pf:Priority|'all';af:string}){
  ...
  return(<div><div className="mb-4 flex items-center gap-3"><div className="flex-1 h-2 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden"><div className="h-full rounded-full bg-blue-500 transition-all duration-500" style={{width:(cc/act.length)*100+'%'}}/></div><span className="text-sm font-medium text-gray-500 dark:text-gray-400">{cc}/{act.length} complete</span></div>
    <div className="space-y-3">{act.map(task=>{const ic=cids.has(task._id);const com=comp.find(x=>x.taskId===task._id);return(<div key={task._id} className={"group relative overflow-hidden rounded-xl border-2 transition-all duration-200 " + gtc(task.priority,ic) + " shadow-sm hover:shadow-lg"}>
      <div className={"flex items-center gap-3 px-5 py-4 " + (ic?"opacity-70":"")}>
        ...
        <span className={"flex-1 min-w-0 text-base font-semibold truncate " + (ic?"text-gray-400 dark:text-gray-500 line-through":"text-gray-900 dark:text-white")}>{task.title}</span>
        <span className={"flex-shrink-0 inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wide " + (task.priority==="red"?"bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400":task.priority==="amber"?"bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400":"bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400")}>{task.priority==="red"?"🔴 High":task.priority==="amber"?"🟡 Medium":"🟢 Low"}</span>
        {task.link&&<a href={task.link} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} className="flex-shrink-0 inline-flex items-center gap-1 rounded-full bg-blue-50/70 dark:bg-blue-900/15 px-3 py-1 text-xs font-medium text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-all hover:shadow-sm"><svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/></svg> Link</a>}
        <select value={cb[task._id]||task.assignedTo} onChange={e=>setCb(prev=>({...prev,[task._id]:e.target.value}))} onClick={e=>e.stopPropagation()} className={"flex-shrink-0 text-xs rounded-full border px-3 py-1 font-medium transition-all " + (ic?"border-gray-200/50 dark:border-gray-700/50 text-gray-400 dark:text-gray-500 bg-gray-50/50 dark:bg-gray-800/50":"border-gray-200/70 dark:border-gray-700/70 text-gray-500 dark:text-gray-400 bg-white/50 dark:bg-gray-800/50 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50/50 dark:hover:bg-blue-900/10")}>{ae.map(n=><option key={n} value={n}>{n}</option>)}</select>
        {ic&&com&&<span className="flex-shrink-0 text-xs text-gray-400 dark:text-gray-500 font-medium whitespace-nowrap">✓ {com.completedBy}</span>}
