...
function TaskList({type,day,dateStr,pf}:{type:TaskType;day:number;dateStr:string;pf:Priority|'all'}){
  ...
  return(<div><div className="mb-4 flex items-center gap-3"><div className="flex-1 h-2 rounded-full bg-gray-200 dark:bg-gray-700 overflow-hidden"><div className="h-full rounded-full bg-blue-500 transition-all duration-500" style={{width:(cc/act.length)*100+'%'}}/></div><span className="text-sm font-medium text-gray-500 dark:text-gray-400">{cc}/{act.length} complete</span></div>
    <div className="space-y-3">{act.map(task=>{const ic=cids.has(task._id);const com=comp.find(x=>x.taskId===task._id);const bc=task.priority==='red'?'border-red-200/60 dark:border-red-900/40 bg-gradient-to-r from-red-50/70 via-red-50/40 to-transparent dark:from-red-950/30 dark:via-red-950/15':task.priority==='amber'?'border-amber-200/60 dark:border-amber-900/40 bg-gradient-to-r from-amber-50/70 via-amber-50/40 to-transparent dark:from-amber-950/30 dark:via-amber-950/15':'border-green-200/60 dark:border-green-900/40 bg-gradient-to-r from-green-50/70 via-green-50/40 to-transparent dark:from-green-950/30 dark:via-green-950/15'};const bgCls=ic?'opacity-70 border-gray-100/30 dark:border-gray-800/30 bg-gray-50/20 dark:bg-gray-900/20':bc;return(<div key={task._id} className={`group relative overflow-hidden rounded-xl border-2 transition-all duration-200 ${bgCls} shadow-sm hover:shadow-lg`}>
